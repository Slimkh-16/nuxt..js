...
const actions = {
  ...
  fetchProductList ({ commit, state }, [filters = {}, redirect]) {
    return new Promise((resolve, reject) => {
      ...
      axios.get(`${PRODUCT_LIST_URL}?${catId}&${qs.stringify(filters)}`)
        ...
        .catch(error => {
          commit(types.PRODUCT_LIST_FETCH_ERROR, { error: error.response ? error.response.status : 404 })
          reject(new Error(`Status code ${error.response ? error.response.status : 404}`))
  ...
  fetchProduct ({ commit, state }, [productAlias, redirect]) {
    return new Promise((resolve, reject) => {
