      ...
      <div class="header-top clearfix">
        <div class="container">
          <div class="header-top__left">
            <ul>
              <li v-for="page in contentPages"  v-if="page.is_header"><router-link :to="{ name: 'Content page' , params : {alias : page.alias}}">{{page.title}}</router-link></li>
            </ul>
          </div>
        ...
        <div class="container">
          ...
          <div class="header-search">
            ...
            <div v-show="subString.length > 3" class="fast-search">
              ...
              <div v-if="searchProductList.length" class="all-results" @click.prevent="searchFunction()">Все результаты</div>
              ...
              <li v-for="link in menu">
                ...
                <ul :id="`drop-catalog-${link.id}`" class="dropdown-content">
                  <li v-for="subLink in link.children">
                    <router-link :to="{ name: 'Products subCategory', params: { 'subCategory' : subLink.alias, 'alias': link.alias } }" :data-activates="`drop-catalog-sublevel-${subLink.id}`" class="dropdown-head-nav">{{subLink.locale.name}}</router-link>
                    <ul v-if="subLink.children.length" :id="`drop-catalog-sublevel-${subLink.id}`" class="dropdown-content dropdown-content--sublevel">
                      <li v-for="subLinkChild in subLink.children">
                        <router-link :to="{ name: 'Products subCategoryChild', params: { 'subCategory' : subLink.alias, 'alias': link.alias, 'subCategoryChild': subLinkChild.alias } }">
            ...
            <li v-for="link in menu" :class="{ 'with-menu': link.children }">
              ...
              <ul>
                <li v-for="subLink in link.children" class="with-menu" :class="{ 'with-menu': subLink.children.length }">
                  <router-link :to="{ name: 'Products subCategory', params: { 'subCategory' : subLink.alias, 'alias': link.alias } }" :class="{ 'with-menu-lnk': subLink.children.length }">
                    {{subLink.locale.name}}
                  </router-link>
...
export default {
  ...
  methods: {
    ...
    searchFunction () {
      /* this.searchByString(this.subString) */
